...
pub type MatcherBroadcastCache = HashMap<Uuid, broadcast::Sender<SubscriptionEvent>>;
...
fn error_to_query_event_bytes_with_meta<E: ToCompactString>(
    ...
    SubscriptionEvent {
        buff: error_to_query_event_bytes(buf, e),
        meta: QueryEventMeta::Error,
    }
}
    ...
    let queue_task = tokio::spawn({
        ...
        async move {
            loop {
                ...
                if let QueryEventMeta::Change(change_id) = seve.meta {
                    if let Err(_e) = queue_tx.try_send((seve.buff, change_id)) {
...
async fn forward_bytes_to_body_sender(
    ...
    loop {
        tokio::select! {
            ...
            res = rx.recv() => {
                match res {
                    Some(event) => {
                        if let Err(e) = handle_sub_event(sub_id, &mut buf, event, &mut tx, &mut last_change_id).await {
    ...
    while let Ok(event) = rx.try_recv() {
        if let Err(e) =
            handle_sub_event(sub_id, &mut buf, event, &mut tx, &mut last_change_id).await
        {
            warn!(%sub_id, "could not forward subscription query event to receiver: {e}");
            return;
        }
